...
// ÔöÇÔöÇ Flag image ÔÇö exactly how Hotmart page does it ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
// s.flag is 'br','co','ar','' etc ÔÇö empty string = unknown = no flag
function getStudentFlag(flag: string, size = 18) {
  if (!flag) return null;
  return getFlagImg(flag, size);
}

// ÔöÇÔöÇ Manual student type (from DB) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
// ÔöÇÔöÇ Glossy table style ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
const TABLE_STYLE: React.CSSProperties = {
  ...
  borderRadius: 24,
  // NOTE: no backdropFilter here ÔÇö it creates a stacking context that traps fixed-position children
};
...
// ÔöÇÔöÇ Payment Status ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
function getPayStatus(s: Student): PayStatus {
  // 0. Manual-only student (no Hotmart history) ÔåÆ base = ADIMPLENTE so bp_em_dia controls it
  if ((s as any).source === 'manual' && s.paymentHistory.length === 0) return 'ADIMPLENTE';
  // 1. ONE_TIME (PIX, Boleto, card ├á vista) ÔåÆ QUITADO
  if (!s.paymentIsSub && !s.paymentIsSmartInstall && s.paymentInstallments <= 1) return 'QUITADO';
  // 2. Standard bank card installments ÔåÆ QUITADO (Hotmart received full amount, bank splits)
...
function PaymentCell({ s, statusOverride }: { s: Student; statusOverride?: 'ADIMPLENTE' | 'INADIMPLENTE' | 'QUITADO' }) {
  ...
  const modeInfo = s.paymentIsSub
    ? `Assinatura ┬À Ciclo ${s.paymentRecurrency}`
    : s.paymentIsSmartInstall
      ? `Parcelamento ┬À ${paid}/${inst}`
      : s.paymentIsCardInstall
        ? `${method} ┬À ${inst}├ù (banco)`
        : method;
  ...
  if (status === 'QUITADO') return (
    <div className="flex flex-col gap-1.5">
      ...
      <span className="text-[9px] uppercase tracking-wider" style={{ color: SILVER }}>
        {s.paymentIsSmartInstall ? `${inst}/${inst} parcelas` : s.paymentIsCardInstall ? `${method} ┬À ${inst}├ù` : method}
      </span>
    </div>
  );
...
// ÔöÇÔöÇ Tooltip ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
function NameTooltip({ s, onHoverIn, onHoverOut }: {
  ...
  if ((isSub || isSmartInstall) && s.lastPayDate && status !== 'QUITADO') {
    for (let i = 1; i <= (isSub ? 3 : inst - actualPaid); i++) {
      upcoming.push({
        ...
        label: isSub ? 'M├¬s estimado' : `Parcela ${actualPaid + i}/${inst}`,
      });
    }
  }
  ...
  const modeLabel = s.paymentIsSub ? `Assinatura ┬À Ciclo ${s.paymentRecurrency}`
    : isSmartInstall ? `Parcel. Inteligente ${actualPaid}/${inst}`
    : s.paymentIsCardInstall ? `${s.paymentLabel} ┬À ${inst}├ù`
    ...
    <div
      ...
      <div className="px-5 py-3" style={{ borderBottom: upcoming.length > 0 ? '1px solid rgba(255,255,255,0.07)' : undefined, maxHeight: 240, overflowY: 'auto' }}>
        <p className="text-[9px] font-black uppercase tracking-widest mb-2" style={{ color: SILVER }}>
          Ô£ô J├í Pagou ({paid.length})
        </p>
        {paid.length === 0
          ? <p className="text-[10px]" style={{ color: SILVER }}>Sem registros</p>
          : paid.map((p, i) => {
              // Use p.index (sequential position) for label ÔÇö avoids duplicate "Parcela 1/Parcela 1"
              // when recurrencyNumber is 1 for multiple separate transactions
              const label = isSub ? `M├¬s ${p.index}` : inst > 1 ? `Parcela ${p.index}` : 'Pago';
              return (
      ...
      {upcoming.length > 0 && (
        <div className="px-5 py-3">
          <p className="text-[9px] font-black uppercase tracking-widest mb-2" style={{ color: SILVER }}>
            ÔùÀ Falta Pagar ({upcoming.length})
          </p>
...
// ÔöÇÔöÇ PDF ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
function effectiveStatusFor(s: Student, bpCache: Record<string, Record<string, any>> = {}): PayStatus {
  ...
  if (rawEmDia != null && rawEmDia !== '') {
    ...
    // Any other value (NAO, N├âO, ATRASADO, free text, etc.) = INADIMPLENTE
    return 'INADIMPLENTE';
  }
...
function generatePDF(
  ...
  const rows = students.map((s, i) => {
    ...
    const stLabel = status === 'INADIMPLENTE' ? 'ÔÜá INADIMPLENTE' : status === 'QUITADO' ? (s.paymentIsSub && s.subStatus === 'CANCELLED' ? 'Encerrado' : 'Ô£ô QUITADO') : 'ÔùÅ ADIMPLENTE';
    ...
    const method   = s.paymentIsSub ? `Assinatura ┬À ${paid} pgtos` : inst > 1 ? `Cart├úo ${inst}├ù ┬À ${paidCard}/${inst}` : 'Pago';
    ...
    // Buyer persona ÔÇö from cache (works for Hotmart + manual)
    ...
    const emDia    = bp.em_dia ? (bp.em_dia === 'SIM' ? 'Ô£ô Em Dia' : 'Ô£ù Atrasado') : '';
    const dadosPessoais = [
      `<b>${s.email}</b>`,
      phone ? `<span style="color:#16a34a">­ƒô× ${phone}</span>` : '',
      cpf   ? `<span style="color:#0369a1">­ƒ¬¬ ${cpf}</span>`  : '',
    ].filter(Boolean).join('<br/>');
    return `<tr style="background:${rowBg}"><td style="color:#888;text-align:center">${i+1}</td><td><strong>${s.name.toUpperCase()}</strong></td><td>${dadosPessoais}</td><td>${fmtDate(s.entryDate)}</td><td style="color:#92400e;font-weight:700">${vendedor}</td><td>${bpPag}${bpModelo ? ` ┬À ${bpModelo}` : ''}</td><td style="font-weight:700">${bpParcela || fmtMoney(vParcela)}</td><td>${fmtMoney(vTotal)}</td><td style="color:${stColor};font-weight:900">${stLabel}</td><td style="color:#888">${emDia}</td><td style="color:#555">${method}</td></tr>`;
  }).join('');
  ...
  if (!win) return;
  win.document.write(`<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"/><title>${courseName} ÔÇö Alunos</title>
...
<div class="ftr">RadExperts Data Center ┬À Dados vital├¡cios</div>
<script>window.onload=()=>window.print()</script></body></html>`);
  win.document.close();
}

// ÔöÇÔöÇ CSV Export ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
function generateCSV(
  ...
  const headers = [
    ...
    'N┬║ PARCELAS','PAGAS','RESTANTES',
    'MOEDA','TURMA',
    // Buyer Persona
    'VENDEDOR','BP VALOR TOTAL','BP PAGAMENTO','BP MODELO','BP PARCELA',
    '1┬¬ PARCELA','├Ült. PAGAMENTO','PR├ôX. PAGAMENTO','EM DIA',
  ];
  const rows = students.map((s, i) => {
...
// ÔöÇÔöÇ XLS Export (SheetJS) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
function generateXLS(
  ...
  /* ÔöÇÔöÇ Sheet 1: Alunos ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */
  const headers = [
    '#', 'NOME', 'EMAIL', 'TELEFONE', 'CPF',
    ...
    // Buyer Persona
    'VENDEDOR', 'BP VALOR TOTAL', 'BP PAGAMENTO', 'BP MODELO', 'BP PARCELA',
    '1┬¬ PARCELA', '├£LT. PAGAMENTO', 'PR├ôX. PAGAMENTO', 'EM DIA',
  ];
  ...
  const data = students.map((s, i) => {
    ...
    return [
      ...
      bp.pagamento || (
        ...
        s.paymentIsCardInstall  ? `Cart├úo (${inst}x banco)` :
        (s.paymentLabel || s.paymentType || '')
      ),
      +vParc.toFixed(2),
      +vTotal.toFixed(2),
  ...
  /* ÔöÇÔöÇ Sheet 2: Resumo ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */
  ...
  const resData: any[][] = [
    ...
    [],
    ['Total Pago (hist├│rico)', +totalPago.toFixed(2)],
  ...
  /* ÔöÇÔöÇ Download ÔöÇÔöÇ */
  XLSX.writeFile(wb, `${courseName.replace(/[^a-z0-9]/gi, '_')}_alunos.xlsx`);
}

// ÔöÇÔöÇ Grid ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
// ÔöÇÔöÇ Detect country flag + currency from phone prefix ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
// ÔöÇÔöÇ Convert ManualStudent ÔåÆ Student shape ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
function manualToStudent(ms: ManualStudent): Student {
  ...
  return {
    ...
    paymentMethod: ms.payment_type === 'PIX' ? 'PIX' : 'Cart├úo',
    paymentLabel: ms.payment_type === 'PIX' ? 'PIX Avulso' : `Cart├úo ${Number(ms.installments)}├ù`,
...
// ÔöÇÔöÇ Add Student Modal ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
// Payment detection ÔÇö ORDER MATTERS: most specific first
function detectPayment(text: string): string {
  ...
  if (t.includes('cartao') || t.includes('card'))    return 'CARTAO_CREDITO'; // bare "cartao" = cr├®dito
...
function parseSingleLine(raw: string): ParsedRow {
  ...
  work = work.replace(/\b(pix|boleto|cart[a├ú]o|cr[e├®]dito|d[e├®]bito|picpay|transfer[e├¬]ncia)\b/gi, ' ');
  ...
  const instMatch = raw.match(/\b(\d{1,2})\s*[xX├ù]\b|\b[xX]\s*(\d{1,2})\b|\b(\d{1,2})\s+vez(?:es)?\b/i);
...
function parseBatchText(text: string): ParsedRow[] {
  ...
  // ÔöÇ Strategy A: blank-line-separated blocks (each block = one student) ÔöÇÔöÇÔöÇÔöÇÔöÇ
  if (/\n[ \t]*\n/.test(text)) {
  ...
  // ÔöÇ Strategy B: one student per line (strip leading commas) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  const rawLines = text
    .split(/\r?\n/)
    .map(l => l.trim().replace(/^[,;\s]+/, ''))
    .filter(Boolean);
...
function ConfBadge({ level }: { level: 'high' | 'medium' | 'low' }) {
  const cfg = level === 'high'   ? { c: '#4ade80', bg: 'rgba(74,222,128,0.1)',  label: 'Ô£ô OK' }
            : level === 'medium' ? { c: '#fbbf24', bg: 'rgba(251,191,36,0.1)',  label: 'ÔÜá Rev' }
            :                      { c: '#f87171', bg: 'rgba(248,113,113,0.1)', label: 'Ô£ò Erro' };
...
// ÔöÇÔöÇ Dup status badge ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
function DupBadge({ status }: { status: 'new' | 'enrich' }) {
  return status === 'new'
    ? <span style={{ fontSize: 9, fontWeight: 900, padding: '2px 7px', borderRadius: 99,
        background: 'rgba(74,222,128,0.1)', color: '#4ade80', whiteSpace: 'nowrap' }}>­ƒåò Novo</span>
    : <span style={{ fontSize: 9, fontWeight: 900, padding: '2px 7px', borderRadius: 99,
        background: 'rgba(99,179,237,0.1)', color: '#63b3ed', whiteSpace: 'nowrap' }}>­ƒöä Atualizar</span>;
}

// ÔöÇÔöÇ CSV / XLS Import Modal ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
type CsvField =
...
const CSV_FIELD_LABELS: Record<CsvField, string> = {
  ...
  bp_ultimo_pagamento:  '├Ültimo Pagamento',
  bp_proximo_pagamento: 'Pr├│ximo Pagamento',
  bp_em_dia:            'Em Dia',
  _ignore:              'ÔÇö Ignorar ÔÇö',
};
...
// Parse a raw Excel serial date or string date ÔåÆ YYYY-MM-DD
...
function CSVImportModal({ courseName, existingEmails, onClose, onSaved }: {
  ...
  const [progress,   setProgress]   = React.useState(0); // 0ÔÇô100 for loading bar
  ...
  const buildRows = (): ParsedRow[] => {
    ...
    return allRaw.map(row => {
      ...
      return {
        ...
        // buyer_persona fields ÔÇö raw strings, API will parse
        ...
        bp_em_dia:            pickRaw('bp_em_dia', row).toUpperCase() === 'SIM' ? 'SIM'
                                : pickRaw('bp_em_dia', row).toUpperCase() === 'NAO' || pickRaw('bp_em_dia', row).toUpperCase() === 'N├âO' ? 'N├âO'
  ...
  const handleImport = async () => {
    ...
    if (rows.length === 0) { setError('Nenhuma linha v├ílida (nome + email obrigat├│rios).'); return; }
    setSaving(true); setProgress(5); setError('');

    // Animate progress bar: fills 5ÔåÆ85% during upload, then 100% on completion
  ...
  return createPortal(
    <div style={{ position: 'fixed', inset: 0, zIndex: 10003, display: 'flex', alignItems: 'center',
      ...
      <div style={BOX}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          ...
          <div>
            ...
            <p style={{ color: SILVER, fontSize: 11, margin: 0, marginTop: 2 }}>
              {step === 'upload'  ? 'Fa├ºa upload da planilha e mapeie os campos'            :
               step === 'map'    ? `${headers.length} colunas detectadas ÔÇö associe os campos` :
               step === 'preview'? `${totalValid} aluno${totalValid !== 1 ? 's' : ''} prontos para importar` :
               'Importa├º├úo conclu├¡da'}
            </p>
          </div>
        ...
        {/* ÔöÇÔöÇ STEP 1: UPLOAD ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
        {step === 'upload' && (<>
          <div
          ...
          <div style={{ padding: '12px 16px', borderRadius: 12, background: 'rgba(255,255,255,0.03)',
            ...
            <p style={{ fontSize: 11, color: SILVER, margin: 0, lineHeight: 1.7 }}>
              A planilha pode ter qualquer cabe├ºalho ÔÇö voc├¬ vai associar cada coluna ao campo correto na pr├│xima etapa. <br/>
              Campos reconhecidos: <strong style={{ color: 'white' }}>Nome, Email, Telefone, CPF, Data de Entrada, Tipo de Pagamento, Valor Parcela, N┬║ Parcelas, Parcelas Pagas</strong>.
            </p>
          </div>
          {error && <p style={{ color: '#f87171', fontSize: 11, marginTop: 8 }}>{error}</p>}
        </>)}

        {/* ÔöÇÔöÇ STEP 2: MAP COLUMNS ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
        {step === 'map' && (<>
          ...
          {rawRows.length > 0 && (
            <div style={{ marginBottom: 20 }}>
              <p style={{ fontSize: 10, fontWeight: 900, color: SILVER, textTransform: 'uppercase',
                letterSpacing: '0.1em', margin: '0 0 8px' }}>Pr├®-visualiza├º├úo (3 primeiras linhas)</p>
                    ...
                    <tr style={{ background: 'rgba(255,255,255,0.04)', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                      {headers.map((h, i) => (
                        <th key={i} style={{ padding: '5px 8px', fontWeight: 800, color: mapping[h] !== '_ignore' ? BLUE : 'rgba(255,255,255,0.3)',
                          ...
                          {h}{mapping[h] !== '_ignore' ? ` ÔåÆ ${CSV_FIELD_LABELS[mapping[h]]}` : ''}
                        </th>
                      ))}
                    </tr>
                  </thead>
        ...
        {/* ÔöÇÔöÇ STEP 3: PREVIEW ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
        {step === 'preview' && (<>
          <div style={{ overflowX: 'auto', marginBottom: 16 }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
                  {['#','Nome','Email','Telefone','CPF','Pagamento','Valor (R$)','Parcelas','Data Entrada','Vendedor','Valor BP','Pag. BP','Modelo','Parcela BP','1┬¬ Parcela','├Ült. Pag.','Pr├│x. Pag.','Em Dia','Status','OK'].map(h => (
              ...
              <tbody>
                {buildRows().map((r, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    ...
                    <td style={{ padding: '5px 8px', color: 'white', fontWeight: 700 }}>{r.name || <span style={{color:'#f87171'}}>ÔÇö</span>}</td>
                    <td style={{ padding: '5px 8px', color: SILVER }}>{r.email || <span style={{color:'#f87171'}}>ÔÇö</span>}</td>
                    <td style={{ padding: '5px 8px', color: SILVER }}>{r.phone || 'ÔÇö'}</td>
                    <td style={{ padding: '5px 8px', color: SILVER }}>{r.cpf || 'ÔÇö'}</td>
                    <td style={{ padding: '5px 8px', color: GOLD, fontWeight: 700 }}>
                      {{PIX:'PIX',BOLETO:'Boleto',CARTAO_CREDITO:'Cr├®dito',CARTAO_DEBITO:'D├®bito'}[r.paymentMethod] || r.paymentMethod}
                    </td>
                    <td style={{ padding: '5px 8px', color: SILVER }}>{r.totalAmount || 'ÔÇö'}</td>
                    <td style={{ padding: '5px 8px', color: SILVER }}>{r.installments}├ù</td>
                    <td style={{ padding: '5px 8px', color: SILVER, whiteSpace:'nowrap' }}>{r.entryDate || 'ÔÇö'}</td>
                    <td style={{ padding: '5px 8px', color: GOLD, fontWeight: 700 }}>{r.vendedor || 'ÔÇö'}</td>
                    <td style={{ padding: '5px 8px', color: SILVER }}>{r.bp_valor || 'ÔÇö'}</td>
                    <td style={{ padding: '5px 8px', color: SILVER }}>{r.bp_pagamento || 'ÔÇö'}</td>
                    <td style={{ padding: '5px 8px', color: SILVER }}>{r.bp_modelo || 'ÔÇö'}</td>
                    <td style={{ padding: '5px 8px', color: SILVER }}>{r.bp_parcela || 'ÔÇö'}</td>
                    <td style={{ padding: '5px 8px', color: SILVER, whiteSpace:'nowrap' }}>{r.bp_primeira_parcela || 'ÔÇö'}</td>
                    <td style={{ padding: '5px 8px', color: SILVER, whiteSpace:'nowrap' }}>{r.bp_ultimo_pagamento || 'ÔÇö'}</td>
                    <td style={{ padding: '5px 8px', color: SILVER, whiteSpace:'nowrap' }}>{r.bp_proximo_pagamento || 'ÔÇö'}</td>
                    <td style={{ padding: '5px 8px' }}>
                      {r.bp_em_dia === 'SIM'
                        ? <span style={{ fontSize: 9, fontWeight: 900, padding: '2px 6px', borderRadius: 99, background: 'rgba(74,222,128,0.1)', color: '#4ade80' }}>Ô£ô SIM</span>
                        : r.bp_em_dia
                        ? <span style={{ fontSize: 9, fontWeight: 900, padding: '2px 6px', borderRadius: 99, background: 'rgba(239,68,68,0.1)', color: '#f87171' }}>Ô£ù {r.bp_em_dia}</span>
                        : <span style={{ color: SILVER }}>ÔÇö</span>}
                    </td>
          ...
          <p style={{ fontSize: 11, color: SILVER, marginBottom: 14 }}>
            <strong style={{ color: 'white' }}>{totalValid}</strong> aluno{totalValid !== 1 ? 's' : ''} v├ílidos de {allRaw.length} linhas
            {' '}({buildRows().filter(r => r.dupStatus === 'enrich').length > 0 && (
              <span style={{ color: '#63b3ed' }}>
                {buildRows().filter(r => r.dupStatus === 'enrich').length} ­ƒöä ser├úo enriquecidos
              </span>
            )})
          </p>
          {/* Loading bar */}
          {saving && (
            <div style={{ marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 10, color: 'rgba(168,178,192,0.6)', margin: '5px 0 0', textAlign: 'center' }}>
                Processando {totalValid} aluno{totalValid !== 1 ? 's' : ''}... n├úo feche esta janela.
              </p>
            </div>
          )}
          {error && <p style={{ color: '#f87171', fontSize: 11, marginBottom: 10 }}>{error}</p>}
        ...
        {/* ÔöÇÔöÇ STEP 4: DONE ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
        {step === 'done' && result && (<>
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            {result.enriched > 0 && (
              <p style={{ color: BLUE, fontSize: 14, fontWeight: 700, margin: '8px 0 4px' }}>
                ­ƒöä {result.enriched} cadastro{result.enriched !== 1 ? 's' : ''} enriquecido{result.enriched !== 1 ? 's' : ''} com dados faltantes
              </p>
            )}
...
// ÔöÇÔöÇ Batch Add Modal ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
function BatchAddModal({ courseName, existingEmails, onClose, onSaved }: {
  ...
  const handleImport = async () => {
    ...
    if (valid.length === 0) { setError('Nenhuma linha v├ílida (nome + email obrigat├│rios).'); return; }
  ...
  const PAYMENT_OPTS = [
    { v: 'PIX',           l: 'PIX' },
    { v: 'CARTAO_CREDITO', l: 'Cart├úo Cr├®dito' },
    { v: 'CARTAO_DEBITO',  l: 'Cart├úo D├®bito' },
    { v: 'BOLETO',         l: 'Boleto' },
  ];
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          ...
          <div>
            ...
            <p style={{ color: SILVER, fontSize: 11, margin: 0, marginTop: 2 }}>
              {step === 'paste'   ? 'Cole os dados ÔÇö a plataforma identifica os campos automaticamente' :
               step === 'preview' ? `${rows.length} aluno${rows.length !== 1 ? 's' : ''} identificado${rows.length !== 1 ? 's' : ''} ÔÇö revise e confirme` :
               'Importa├º├úo conclu├¡da'}
            </p>
          </div>
        ...
        {/* ÔöÇÔöÇÔöÇ STEP 1: PASTE ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
        {step === 'paste' && (<>
          {/* Examples */}
          <div style={{ marginBottom: 16, padding: '12px 14px', borderRadius: 12,
            ...
            {["Jo├úo Silva, joao@email.com, (11) 99999-0000, 123.456.789-00, PIX",
              "Maria Souza\naria.souza@gmail.com\n(21) 98765-4321\nCart├úo Cr├®dito",
              "joao@email.com 11 99999-0000 Jo├úo da Silva 123.456.789-00 boleto",
            ].map((ex, i) => (
          ...
          <textarea
            ...
            placeholder={"Jo├úo Silva, joao@gmail.com, (11) 99999-0000\nMaria Santos, maria@hotmail.com, 987.654.321-00, Boleto\n..."}
        ...
        {/* ÔöÇÔöÇÔöÇ STEP 2: PREVIEW ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
        {step === 'preview' && (<>
          <div style={{ overflowX: 'auto', marginBottom: 16 }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
              <thead>
                          ...
                          <td colSpan={numCols - 1} style={{ padding: '4px 4px 8px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                              <span style={{ fontSize: 9, fontWeight: 900, color: GOLD, textTransform: 'uppercase',
                                letterSpacing: '0.1em', marginRight: 4, whiteSpace: 'nowrap' }}>­ƒÆ│ Parcelamento:</span>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                <label style={{ fontSize: 9, color: SILVER, whiteSpace: 'nowrap' }}>N┬║ Parcelas</label>
                              ...
                              <span style={{ color: SILVER, fontSize: 11 }}>├ù</span>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          ...
          <div style={{ padding: 16, background: 'rgba(255,255,255,0.03)', borderRadius: 12, border: '1px solid rgba(255,255,255,0.08)', marginBottom: 20 }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, fontWeight: 900, color: SILVER, textTransform: 'uppercase', letterSpacing: '0.1em', cursor: 'pointer' }}>
              ...
              Incluir Informa├º├Áes Adicionais para TODOS OS ALUNOS do lote (Modelo / Vendedor / Status)
            </label>
            {useBp && (
        ...
        {/* ÔöÇÔöÇÔöÇ STEP 3: DONE ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
        {step === 'done' && result && (<>
          <div style={{ textAlign: 'center', padding: '12px 0' }}>
            ...
            {(result as any).enriched > 0 && (
              <p style={{ color: '#63b3ed', fontSize: 13, margin: '4px 0' }}>
                ­ƒöä {(result as any).enriched} cadastro{(result as any).enriched !== 1 ? 's' : ''} enriquecido{(result as any).enriched !== 1 ? 's' : ''} com dados faltantes
              </p>
            )}
      ...
      <div style={{ ...GLASS, position: 'relative', width: '100%', maxWidth: 600, maxHeight: '90vh', overflowY: 'auto', padding: 32 }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <p style={{ fontSize: 10, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.15em', color: 'rgba(168,178,192,0.8)', marginBottom: 12 }}>
            Informa├º├Áes Adicionais (Planilha)
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, marginBottom: 20 }}>
            ...
            <div>
              ...
              <input style={INPUT} placeholder="1x / Recorr├¬ncia" value={form.bp_modelo}
          ...
          <div style={{ marginBottom: 24 }}>
            <label style={LABEL}>Observa├º├Áes</label>
            <textarea style={{ ...INPUT, minHeight: 60, resize: 'vertical' }} placeholder="Anota├º├Áes extras..."
...
// ÔöÇÔöÇ Date format helpers ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
// YYYY-MM-DD  ÔåÆ  DD/MM/AAAA  (for display inside the modal)
...
// DD/MM/AAAA  ÔåÆ  YYYY-MM-DD  (for the backend API)
...
// ÔöÇÔöÇ Date field with auto-mask DD/MM/AAAA ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
...
// ÔöÇÔöÇ Field helper for EditStudentModal (must be at module scope ÔÇö NOT inside render) ÔöÇÔöÇ
...
// ÔöÇÔöÇ Edit Student Modal ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
// Normalise legacy SIM/N├âO values ÔåÆ canonical Adimplente/Inadimplente/Quitado
function normaliseEmDia(v?: string): string {
  const raw = (v || '').trim();
  const u = raw.toUpperCase();
  if (u === 'SIM') return 'Adimplente';
  if (u === 'N├âO' || u === 'NAO' || u === 'N├éO') return 'Inadimplente';
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 10001, display: 'flex', alignItems: 'center',
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          ...
          <div>
            <h3 style={{ color: 'white', fontWeight: 900, fontSize: 15, margin: 0 }}>Editar Informa├º├Áes</h3>
            <p style={{ color: SILVER, fontSize: 11, margin: 0, marginTop: 2 }}>{student.name} ┬À {student.email}</p>
          </div>
          <button onClick={onClose} style={{ marginLeft: 'auto', background: 'none', border: 'none',
            color: SILVER, cursor: 'pointer', padding: 4 }}>
            <span className="material-symbols-outlined" style={{ fontSize: 18 }}>close</span>
        ...
        {/* Date fields */}
        <EditDateField label="1┬¬ Parcela (DD/MM/AAAA)" icon="event" value={bpPrimeira} onChange={setBpPrimeira} />
        <EditDateField label="├Ültimo Pagamento (DD/MM/AAAA)" icon="event_available" value={bpUltimo} onChange={setBpUltimo} />
        <EditDateField label="Pr├│ximo Pagamento (DD/MM/AAAA)" icon="schedule" value={bpProximo} onChange={setBpProximo} />

        {/* Observa├º├Áes */}
        <div style={{ marginBottom: 16, marginTop: 4 }}>
          <label style={{ display: 'block', fontSize: 10, fontWeight: 900, letterSpacing: '0.12em',
            textTransform: 'uppercase', color: SILVER, marginBottom: 6 }}>Observa├º├Áes</label>
          <textarea
            value={notes}
            onChange={e => setNotes(e.target.value)}
            placeholder="Anota├º├Áes extras..."
            style={{
              width: '100%', padding: '10px 12px', borderRadius: 10, fontSize: 13,
              background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.12)',
...
// ÔöÇÔöÇ Delete Confirm Modal ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
      ...
      <div style={{
        ...
        <p style={{ color: SILVER, fontSize: 13, textAlign: 'center', lineHeight: 1.6, margin: '0 0 8px' }}>
          ...
          {isHotmart
            ? 'ser├í ocultado desta lista. Vendas e acesso ├á plataforma n├úo s├úo afetados.'
            : 'ser├í removido permanentemente da lista de alunos manuais.'}
        </p>
        <p style={{ color: 'rgba(168,178,192,0.6)', fontSize: 11, textAlign: 'center', lineHeight: 1.5, margin: '0 0 26px' }}>
          {isHotmart
            ? 'Esta a├º├úo n├úo cancela a venda na Hotmart nem remove o acesso do aluno ao produto.'
            : 'Isso n├úo afeta dados da Hotmart, vendas, assinaturas ou acesso ├á plataforma.'}
        </p>
        <div style={{ display: 'flex', gap: 10 }}>
...
export default function CursoDetailPage({ params }: { params: Promise<{ courseName: string }> }) {
  ...
  // Tooltip ÔÇö single global mouse tracker, no React state for position
  ...
  // One listener tracks mouse and moves tooltip div directly ÔÇö no React state involved
  useEffect(() => {
  ...
  // Resolve slug ÔåÆ real course name, then load students (single sequential flow)
  useEffect(() => {
    fetch('/api/cursos')
      .then(r => r.json())
      .then(d => {
  ...
  // Merge Hotmart + manual students ÔÇö manual has priority (dedup by email)
  ...
  // ÔöÇÔöÇ Pre-load buyerPersonaCache for ALL students on initial load (for correct pill counts) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  useEffect(() => {
  ...
  // ÔöÇÔöÇ Background contact fetch (phone + CPF + buyer_persona) for current page ÔöÇÔöÇ
  useEffect(() => {
    ...
    <LoginWrapper>
      <div style={{ minHeight: '100vh' }}>
        ...
        <main className="px-4 md:px-6 max-w-[1700px] mx-auto pt-10 pb-24">
          ...
          {/* ÔöÇÔöÇ Action toolbar (row 2) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap', marginBottom: 28 }}>

            {/* ÔöÇÔöÇ Group 1: Gerar Relat├│rios ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <p style={{ fontSize: 9, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.15em',
                color: 'rgba(168,178,192,0.6)', margin: 0, paddingLeft: 2 }}>Gerar Relat├│rios</p>
            ...
            {/* ÔöÇÔöÇ Group 2: Importar Alunos ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <p style={{ fontSize: 9, fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.15em',
                color: 'rgba(168,178,192,0.6)', margin: 0, paddingLeft: 2 }}>Importar Alunos</p>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                {/* Adicionar aluno ├║nico */}
          ...
          {/* Search + Status filter pills ÔÇö same row */}
          ...
          {/* Table ÔÇö no overflow:hidden so portaled tooltip renders above all elements */}
          ...
          <div style={{ ...TABLE_STYLE, overflow: 'visible', minWidth: 900 }}>
            ...
            <div className="flex items-center justify-between px-5 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-black uppercase tracking-widest" style={{ color: SILVER }}>Por p├ígina:</span>
              ...
              {totalPages > 1 && (
                <div className="flex items-center gap-2">
                  <button onClick={() => setPage(p => Math.max(0, p - 1))} disabled={page === 0}
                    ...
                    style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', color: SILVER }}>ÔåÉ Anterior</button>
                  <span className="text-[12px] font-bold px-3" style={{ color: SILVER }}>{page + 1} / {totalPages}</span>
                  <button onClick={() => setPage(p => Math.min(totalPages - 1, p + 1))} disabled={page >= totalPages - 1}
                    className="px-4 py-1.5 rounded-xl text-[11px] font-black transition-all disabled:opacity-30"
                    style={{ background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', color: SILVER }}>Pr├│xima ÔåÆ</button>
                </div>
              )}
              ...
              paginated.map((s, idx) => {
                ...
                return (
                  <div key={s.transaction || s.email + idx}
                    ...
                    {/* Name + flag ÔÇö exactly like Hotmart page */}
                    ...
                    <div className="flex flex-col gap-1 pr-3 pt-1">
                      ...
                      {/* Telefone ÔÇö buyer_persona overrides */}
                      {(() => {
                    ...
                    {/* Valor Parcela ÔÇö buyer_persona.valor/parcela overrides Hotmart */}
                    <div className="flex flex-col gap-0.5 pt-1">
                      {(() => {
                        const bp = buyerPersonaCache[(s.email || '').toLowerCase()] || {};
                        // For Hotmart BRL students, trust Hotmart pricing ÔÇö skip bp override
                        ...
                        return (
                          <>
                            ...
                            {s.valorBRL != null && s.currency !== 'BRL' && (
                              <span className="text-[9px] font-bold" style={{ color: SILVER }}>Ôëê {fmtMoney(s.valorBRL)}</span>
                            )}
                          </>
                        );
                      })()}
                    ...
                    <div className="flex flex-col gap-0.5 pt-1">
                      ...
                      {s.valorBRL != null && s.currency !== 'BRL' && (() => {
                        const totalBrl = s.paymentHistory.length > 0
                          ...
                          : s.valorBRL;
                        return <span className="text-[9px] font-bold" style={{ color: SILVER }}>Ôëê {fmtMoney(totalBrl)}</span>;
                      })()}
                    </div>
                    {/* Pagamento ÔÇö buyer_persona overrides Hotmart */}
                    {(() => {
                      const bp = buyerPersonaCache[(s.email || '').toLowerCase()] || {};
                          ...
                          <div className="flex flex-col gap-0.5 pt-1">
                            ...
                            {emDia != null && (
                              <span className="text-[9px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full" style={{
                                ...
                                {emDia === 'SIM' ? 'Ô£ô Em dia' : emDia?.toUpperCase() === 'QUITADO' ? 'Ô£ô Quitado' : 'Ô£ù Atrasado'}
                              </span>
                            )}
                            {bp.proximo_pagamento && (
                              <span className="text-[9px]" style={{ color: SILVER }}>
                                Pr├│x: {new Date(bp.proximo_pagamento).toLocaleDateString('pt-BR')}
                              </span>
                            )}
                          </div>
                        );
                    ...
                    {/* Delete action ÔÇö all students */}
